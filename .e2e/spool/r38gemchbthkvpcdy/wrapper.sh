...
set -u
DIR=/home/dev/projects/jarvis/.e2e/spool/r38gemchbthkvpcdy
...
( export PATH=$HOME/.local/bin:$HOME/.opencode/bin:$HOME/.bun/bin:$HOME/bin:/usr/local/bin:$PATH; cd /tmp/jarvis-demo/app && claude -p '@@slow:3 revisa el pool' --output-format stream-json --verbose --resume sid-pool --permission-mode plan < /dev/null ) >> "$DIR/events.ndjson" 2>> "$DIR/stderr.log" &
PID=$!
printf '%s\n' "$PID" > "$DIR/pid"
publish running null null
