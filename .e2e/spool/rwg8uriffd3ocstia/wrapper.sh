...
set -u
DIR=/home/dev/projects/jarvis/.e2e/spool/rwg8uriffd3ocstia
...
[jarvis] Paso 2: con lo anterior, propón el cambio concreto.' --output-format stream-json --verbose --resume sid-pool --permission-mode plan < /dev/null ) >> "$DIR/events.ndjson" 2>> "$DIR/stderr.log" &
PID=$!
printf '%s\n' "$PID" > "$DIR/pid"
publish running null null
