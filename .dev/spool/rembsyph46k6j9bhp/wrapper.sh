...
set -u
DIR=/home/dev/projects/jarvis/.dev/spool/rembsyph46k6j9bhp
...
( export PATH=$HOME/.local/bin:$HOME/.opencode/bin:$HOME/.bun/bin:$HOME/bin:/usr/local/bin:$PATH; cd /srv/app && claude -p '@@slow:6 revisa por que el pool no devuelve conexiones' --output-format stream-json --verbose --resume sid-pool --permission-mode plan < /dev/null ) >> "$DIR/events.ndjson" 2>> "$DIR/stderr.log" &
PID=$!
printf '%s\n' "$PID" > "$DIR/pid"
publish running null null
