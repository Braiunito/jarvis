...
de decidir traer `react-markdown` contando con un tercio más de margen del que hay. **Cuando se
mueva la cifra de la tabla, se mueve también ésta.**
...
React **nunca produce cadenas de HTML**, así que no hay nada que sanitizar y la clase de fallo
entera deja de existir en vez de quedar sujeta a una configuración. Pero la cifra es la que cierra
la conversación que este ADR pide tener en cada alta.
