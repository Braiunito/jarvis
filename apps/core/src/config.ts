...
/**
 * La configuración a partir de un entorno, en vez de leer el global al importar.
 *
 * Se lee así por una razón que costó una tarde de rojos intermitentes: probarla obligaba a
 * **reemplazar `process.env` entero** y reimportar el módulo, y `process.env` es global al proceso.
 * Con los ficheros de prueba corriendo en paralelo, un caso que sustituye el entorno mientras otro
 * lo lee produce una roja que desaparece al repetir — y esas se atribuyen al ruido y no se
 * investigan. Yo mismo di por buenas varias culpando al árbol compartido.
 *
 * Con el entorno como argumento, probar la configuración no toca nada de nadie. Y de paso deja de
 * ser algo que sólo se puede observar arrancando el proceso entero.
 */
...
/** La del proceso. Lo que usa el core al arrancar. */
export const config = buildConfig();

export type CoreConfig = ReturnType<typeof buildConfig>;
